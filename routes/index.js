...
router.post('/register', function(req, res, next) {
 ...
 con.connect(function(err) {
  if (err) throw err;
  var sql = "INSERT INTO usertbls (email,password,mobileno) VALUES ('"+email+"', '"+password+"','"+mobileno+"')";
...
router.post('/view_device',function(req,res,next){
	...
	var device_loc = req.body.deviceloc;
	var d_id;
	DeviceModel.findOne({},function(err,ddata){
							
							if (ddata) {
								d_id = ddata.d_id + 1;
								req.session.d_id = d_id;
								console.log("devicestbl"+req.session.d_id);
							}else{
								d_id=1;
								req.session.d_id = d_id;
								console.log("device first entry"+req.session.d_id);
							}
							...
							newdevice.save(function(err, Rfid){
								if(err)
									console.log(err);
								else
								{
									console.log('Success in device tbl');
									res.send({"Success":"true"});
